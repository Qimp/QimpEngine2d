...
int AnimationCurveTpl<T>::FindIndex (const Cache& cache, float curveT) const
{
	...
	index--;
	index = std::min<int>(m_Curve.size() - 2, index);
...
template std::pair<float, float> AnimationCurveTpl<T>::GetRange() const; \
template T AnimationCurveTpl<T>::Evaluate (float curveT) const; \
template void AnimationCurveTpl<T>::RemoveKeys (iterator begin, iterator end);\
template int AnimationCurveTpl<T>::AddKey (const Keyframe& key);\
template int AnimationCurveTpl<T>::FindIndex (float time) const; \
template int AnimationCurveTpl<T>::FindIndex (const Cache& cache, float time) const; \
template void AnimationCurveTpl<T>::InvalidateCache (); \
template void AnimationCurveTpl<T>::SetPreInfinity (int mode); \
template void AnimationCurveTpl<T>::SetPostInfinity (int mode); \
template int AnimationCurveTpl<T>::GetPreInfinity () const; \
template int AnimationCurveTpl<T>::GetPostInfinity () const; \
template KeyframeTpl<T>::KeyframeTpl (float time, const T& value);

INSTANTIATE(float)
INSTANTIATE(Vector3f)
INSTANTIATE(Quaternionf)

template void AnimationCurveTpl<float>::CalculateCacheData (Cache& cache, int lhs, int rhs, float timeOffset) const;
template float AnimationCurveTpl<float>::EvaluateClamp (float curveT) const;
template Quaternionf AnimationCurveTpl<Quaternionf>::EvaluateClamp (float curveT) const;
template Vector3f AnimationCurveTpl<Vector3f>::EvaluateClamp (float curveT) const;
template void AnimationCurveTpl<Vector3f>::EvaluateWithoutCache (float curveT, Vector3f& output)const;
...
void RecalculateSplineSlope (AnimationCurveTpl<T>& curve)
{
	for (int i=0;i<curve.GetKeyCount ();i++)
		RecalculateSplineSlopeT (curve, i, 0.0f);
}

template void RecalculateSplineSlope (AnimationCurveTpl<float>& curve);
template void RecalculateSplineSlope (AnimationCurveTpl<Vector3f>& curve);
