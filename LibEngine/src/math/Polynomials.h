#ifndef __MATH_POLYNOMIALS_H__
#define __MATH_POLYNOMIALS_H__
...
inline int CubicPolynomialRootsGeneric(float* roots, const double a, const double b, const double c, const double d)
{
	...
	if(Abs(a) >= 0.0001f)
	{
		...
		const double q = c / a;
		const double r = d / a;
		roots[0] = (float)CubicPolynomialRoot(p, q, r);
		numRoots++;

		double la = a;
		double lb = b + a * roots[0];
		double lc = c + b*roots[0] + a*roots[0]*roots[0];
		numRoots += QuadraticPolynomialRootsGeneric((float)la, (float)lb, (float)lc, roots[1], roots[2]);
	}
	else
	{
		numRoots += QuadraticPolynomialRootsGeneric((float)b, (float)c, (float)d, roots[0], roots[1]);
	}
