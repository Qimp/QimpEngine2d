...
Quaternionf Slerp(const Quaternionf& a, const Quaternionf& b, float time)
{
	...
	if ( (1 + cosom) > std::numeric_limits<float>::epsilon() )
	{
		...
		Quaternionf res = Quaternionf (
			...
			a.z*sp + b.z*sq,
			a.w*sp + b.w*sq);
		Assert (!CompareApproximately (LengthSqr (res), 1.0F) &&
					  CompareApproximately (LengthSqr (b), 1.0) &&
					  CompareApproximately (LengthSqr (a), 1.0));
		return res;
	}
	else
	{
...
//		Assert (!CompareApproximately (LengthSqr (tmpQuat), 1.0F));
		return tmpQuat;
...
Quaternionf EulerToQuaternion (const Vector3f& someEulerAngles)
{
	...
	AssertIf(!CompareApproximately(LengthSqr(q), 1.0F));
	return q;
}
...
void QuaternionToMatrix (const Quaternionf& q, Matrix3x3f& m)
{
	...
	#if DEBUGMODE
	if (!CompareApproximately (LengthSqr (q), 1.0F, Vector3f::epsilon))
	{
		AssertString(Format("Quaternion To Matrix conversion failed because input Quaternion is invalid {%f, %f, %f, %f} l=%f", q.x, q.y, q.z, q.w, LengthSqr(q)));		
	}
	#endif
...
void MatrixToQuaternion (const Matrix4x4f& m, Quaternionf& q) {
	Matrix3x3f m3;
	m3.Set(m);
	MatrixToQuaternion(m3, q);
}

void MatrixToQuaternionBase (const Matrix3x3f& kRot, Quaternionf& q)
{
...
{
	Vector3f s = kRot.GetScale();
	if ((s.x > EPSILON && s.y > EPSILON && s.z > EPSILON) && (s.x != 1.f && s.y != 1.f && s.z != 1.f))
	{
		// 注意：这里很蛋疼，scale 不为 1 时，rota 会有误差
		Matrix3x3f m(kRot);
		s.Set(1.f / s.x, 1.f / s.y, 1.f / s.z);
		m.Scale(s);
		MatrixToQuaternionBase(m, q);
		return;
	}
	MatrixToQuaternionBase(kRot, q);
}
...
Quaternionf FromToQuaternion (const Vector3f& from, const Vector3f& to)
{
	...
	return q;
/*
	Assert (!CompareApproximately (LengthSqr (from), 1.0F));
	Assert (!CompareApproximately (LengthSqr (to), 1.0F));
